...
export function produceFor(rustFile: string, data: dataInfo, outFile: string) {
...
function main() {
    ...
    try {
        const stat = fs.statSync(rustSourceFile)
    } catch (ex) {
    ...
    var execResult = child_process.spawnSync("npm", ["link"], spawnOptions); 
    if (execResult.error) {

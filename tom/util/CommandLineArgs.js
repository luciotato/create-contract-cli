 ...
 * Example:
 ```
> lucio@lucio-kde10:~/repos/near-create-cli$ node dist/lucky deploy ct.wasm lucky.betanet { owner_id:"luciotato.betanet", staking_public_key:"0uewoieu23uo2iu34o234234", { numerator:8, denominator:100 } }

process.argv=
[
  '/usr/local/bin/node',
  '/home/lucio/repos/near-create-cli/dist/lucky',
  'deploy',
  'ct.wasm',
  'lucky.betanet',
  '{',
  'owner_id:luciotato.betanet,',
  'staking_public_key:0uewoieu23uo2iu34o234234,',
  '{',
  'numerator:8,',
  'denominator:100',
  '}',
...
-------------------------------
```
>node dist/lucky transfer account_id:luciotato dest:otheraccount.betanet stake:false --attach 100N
...
## It's Flexible in what receives
-------------------------------
```
node dist/lucky withdraw amount:100N
node dist/lucky withdraw { amount:100N }
node dist/lucky withdraw {amount:100N}
```
all should parse to:
...
### Interpret [ ]
Note: you must use spaces around  [ and ]
lucio@lucio-kde10:~/repos/near-create-cli$ node dist/lucky deploy ct.wasm lucky.betanet [ luciotato.betanet, luciotato.staker-1.betanet ]
[
  '/usr/local/bin/node',
  '/home/lucio/repos/near-create-cli/dist/lucky',
  'deploy',
...
class CommandLineArgs {
    constructor(optionsDeclaration) {
        ...
        //special case "--help"
        if (this.positional.length == 0 && this.options.help) {
            this.command = "--help";
        }
            ...
            //take the first aregument
            this.command = this.positional.shift();
        }
    }
    /**
     ...
     */
    requireOptionString(option) {
        let value = this.options[option.name].toString().trim();
 ...
 * @param optionName option name
 */
    requireOptionAmount(option, units) {
        let value = this.options[option.name].toString().trim();
        if (value == undefined || value == "") {
            console.log(`ERROR: required --${option.name}`);
            process.exit(1);
     ...
     * @param optionName option name
     */
    consumeOption(option, defaultValue) {
     ...
     */
    convertAmount(value, requiredUnits) {
        let result;
    ...
    extractJSONObject(start) {
        ...
        for (let index = 0; index < extracted.length; index++) {
            ...
            if (propValue == undefined || propValue == "") {
                console.log(`ERROR: expected ${propName}:value, received only '${propName}'`);
                process.exit(1);
            }
            if (propValue == "{") { //subornidated object
                const subObj = this.extractJSONObject(index + 1); //recursive
                propValue = subObj.value;
                index = subObj.end; //skip internal object
            }
            if (propValue.endsWith(","))
                propValue = propValue.slice(0, propValue.length - 1);
            if (propValue.match(/^[0-9_\.]*$/)) { //it's a number
    ...
    */
    ...
    searchOption(option) {
        ...
        ;
        //store found positon into this.lastIndex
        this.lastIndex = this.search(variants);
        return this.lastIndex;
    }
    // ---------------------------
    /**
     * search for an option by several names in the command line args
     * return position in the array where it was found|-1
     */
    search(variants) {
        //for each item in list
        variants.forEach(item => {
            var result = this.clArgs.indexOf(item);
            if (result >= 0) {
                return result; //found
            }
        });
